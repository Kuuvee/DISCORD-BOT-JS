  ...
  try {
    ...
    for (let i = 0; i < queue.songs.length; i += 10) {
      ...
      const embed = new Discord.MessageEmbed()
        ...
        .setDescription(
          `**เพลงที่เล่นอยู่ - [\`${qus[0].name}\`](${qus[0].url})**\n\n${info}`
        )
      embeds.push(embed);
    }
    //returning the Embed
...
async function playsongyes(client, message, queue, song) {
  try {
    let embed1 = new Discord.MessageEmbed()
      .setColor(config.colors.yes)
      .setTitle("เพลงกำลังเล่น!")
      .setDescription(`เพลง: [\`${song.name}\`](${song.url})`)
      .addField(
        "คิวเพลง:",
        `>>> \`${queue.songs.length} เพลง - ${queue.formattedDuration}\``,
        true
      )
      ...
      .setFooter("เล่นเพลงโดย : Qlix",'https://i.imgur.com/AfFp7pu.png')
